...
classdata=json.loads(open('.data/data.json').read()) #class_name and position
...
def sendmail(fn='message'):
    ...
    receiver_email=''
    msg_template = read_template('.data/'+fn+'.txt')
    if fn=='message':
    ...
    with smtplib.SMTP(smtp_server, port) as server:
        ...
        print(message)
        for email in get_contacts('.data/contacts.txt'):
